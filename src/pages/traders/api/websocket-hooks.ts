import { DeciDashConfig, type WebsocketResponseMarketDepth, type WebsocketResponseMarketPrice } from '@coldbell/decidash-ts-sdk';
...
export const useMarketPriceStream = (
  ...
  const [latestPrice, setLatestPrice] = useState<WebsocketResponseMarketPrice['price'] | null>(null);
  const [isConnected, setIsConnected] = useState(false);
  const [error, setError] = useState<Error | null>(null);

  const queryKey: QueryKey = ['marketPriceStream', marketSymbol];
  ...
  useEffect(() => {
    ...
    const { connect, disconnect, subscribeMarketPrice } = getWsSession(config);
    let cancelled = false;
...
export const useMarketDepthStream = (
  ...
  const queryKey: QueryKey = ['marketDepthStream', marketSymbol, depth];
  ...
  useEffect(() => {
    ...
    const { connect, disconnect, subscribeMarketDepth } = getWsSession(config);
    let cancelled = false;
      ...
      try {
        ...
        for await (const msg of stream as AsyncIterable<WebsocketResponseMarketDepth>) {
          ...
          const mappedBids = nextBids.map(l => {
            acc += l.size;
            return { price: l.price, size: l.size, total: acc };
          });
          ...
          const bestBid = mappedBids.length > 0 ? mappedBids[0].price : undefined;
          const bestAsk = mappedAsks.length > 0 ? mappedAsks[0].price : undefined;
          
          const spread = bestBid && bestAsk ? bestAsk - bestBid : undefined;
          const spreadPercent = spread && bestAsk ? (spread / bestAsk) * 100 : undefined;
...
export const useCandlestickRealtimeUpdater = (
  ...
  useEffect(() => {
    ...
    const subscribe = async () => {
      try {
        ...
        if (!cancelled) {
          console.error('Candlestick realtime update failed:', error);
        }
      } finally {
        setIsConnected(false);
      }
  ...
  }, [marketSymbol, interval, candlestickQueryKey, config, queryClient]);
