...
import type { DepthLevel } from '../api/queries'
...
export default function TradingOrder() {
  ...
  const marketDepthQuery = useMarketDepth(
    market,
    rowsPerSide,
  )
  const { bids = [], asks = [] } =
    marketDepthQuery.data || {}
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col p-4">
      ...
      >
        ...
        <div className="flex min-h-0 flex-1 flex-col justify-end space-y-1 overflow-hidden pr-1">
          {asks.map((l: DepthLevel, i: number) => (
            <div
              key={`ask-${i}`}
              className="relative grid grid-cols-3 items-center gap-2 text-sm"
            >
              <div
                className="absolute inset-y-0 left-0 bg-green-900/25"
                style={{
                  width: `${Math.min(100, (l.total / maxAskTotal) * 100)}%`,
                }}
              />
        ...
        {/* Bids (아래) - 상단 정렬 */}
        <div className="flex min-h-0 flex-1 flex-col justify-start space-y-1 overflow-hidden pr-1">
          {bids.map((l: DepthLevel, i: number) => (
            <div
              key={`bid-${i}`}
              className="relative grid grid-cols-3 items-center gap-2 text-sm"
            >
              <div
                className="absolute inset-y-0 left-0 bg-red-900/25"
                style={{
                  width: `${Math.min(100, (l.total / maxBidTotal) * 100)}%`,
                }}
              />
